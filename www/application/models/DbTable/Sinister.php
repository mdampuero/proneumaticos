...
class Model_DBTable_Sinister extends Zend_Db_Table_Abstract {
    ...
    public function showAll($where = null, $sort = null, $order = null, $limit = 0) {
        ...
        $select->from(array($this->_name), array("*", "(CASE " . $this->_name . "." . $this->status . " "
            ...
            . " end ) as si_data_complete","CONCAT(datediff('".date("Y-m-d")."',si_date),'') as days"));
        ...
        $select->joinLeft("sg_tbranch as tb_au", "tb_au.tb_id=" . $this->_name . ".si_tb_id_au", array("tb_name as tb_name_au"));
        $select->joinLeft("sg_tbranch as tb_po", "tb_po.tb_id=" . $this->_name . ".si_tb_id_po", array("tb_name as tb_name_po"));
        $select->joinLeft("sg_cbranch as cb", "cb.cb_id=" . $this->_name . ".si_cb_id", array("cb_name as cb_name"));
        $select->joinLeft("sg_bbranch as bb", "bb.bb_id=" . $this->_name . ".si_bb_id", array("bb_name as bb_name"));
